...
async function fileExists(
	name: string,
	parentId?: string
): Promise<[boolean, gapi.client.drive.File?]> {
	const body = { q: `name = '${name}'` };
	if (parentId) body.q = `'${parentId}' in parents and ` + body.q;
...
async function InitializeDriveSpace(
	{ name, children }: FolderTree,
	parent?: string
) {
	const body: any = {
		// @ts-ignore
	...
	if (exist && file) {
		children.forEach((child) => {
			if (file.id) InitializeDriveSpace(child, file.id);
			else
				console.error(
		...
		gapi.client.drive.files.create(body).then((file) => {
			children.forEach((child) => {
				if (file.result.id) InitializeDriveSpace(child, file.result.id);
				else
					console.error(
						`couldn't create file `,
						{ name, children },
...
export default function DriveSetup() {
	if (!gapi || !gapi.client) {
		...
		// 	confirmButtonText: <ConnectionBtn />,
		// });
		// @ts-ignore
	...
	gapi.client.load("drive", "v3", async () => {
		const aTf = (a: Administrator): FolderTree => {
			return {
				name: a.divisionName || a.name,
				children: a.subordinates.map(aTf),
			};
		};
